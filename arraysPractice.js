...
// argument and loops through the given array backwards alerting every item in the
...
//   passed in as a parameter) to see if that random number is in the array.
//   If it is, return true, if it's not, return false

  //Code Here
function finder(numbers){
  return (numbers.indexOf(getRandomArbitrary()) > -1);
}
  //Code Here
...
*/
  ...
  //Code Here
  function removeItem(myGroceryList, rmvItm){
    for(var i = 0; i < myGroceryList.length; i++){
      if(myGroceryList[i] === rmvItm){
  ...
  function addItem(myGroceryList, addItm){
    if(myGroceryList.indexOf(addItm) === -1){
        myGroceryList.push(addItm)
    }
    return myGroceryList;
  }
...
Loop through your devMountainEmployees until you find cahlan, then remove him from the array.*/
  ...
  //Code Here
devMountainEmployees.forEach(function (name, index, array){
  if(name["name"] === "Cahlan"){
